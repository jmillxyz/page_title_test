defmodule PageTitleTestWeb.UserLiveTest do
  use PageTitleTestWeb.ConnCase

  import Phoenix.LiveViewTest
  import PageTitleTest.AccountsFixtures
  ...
  defp create_user(_) do
    user = user_fixture(%{name: "D'Angelo Smith"})
    %{user: user}
  end
  ...
  describe "Show" do
    ...
    test "displays page title", %{conn: conn, user: user} do
      {:ok, show_live, _html} = live(conn, ~p"/users/#{user}")
      ...
      # fails
      assert page_title(show_edit_live) =~ "Edit User: #{user.name}"
    end
  end
end
